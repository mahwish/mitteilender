...
from ..forms import ContactForm, NewForm, ItemForm, MoreItemsForm, FieldForm, EmailForm,AddProjectForm
...
def view(request):
    ...
    k=[]
    project_name = request.matchdict['pname']
    p_id = DBSession.query(Info_projects.ip_id).filter_by(name=project_name)
    #request.session.flash(project_name.ip_id)
    
    p_items = DBSession.query(project_items).filter_by(infoproject_id=p_id).all()
    for i in p_items:
       p.append(int(i.display_order))
    k=sorted(p)
    return {'p_items':p_items,'display_order':k}
...
def tryy(request):
    ...
    project_details = get_items(P.ip_id, '')
    request.session.flash(project_details)
    return{}
...
def swap_display_order(request):
	...
	n.append(str(a))
      ...
      for i in q:
	    ...
	    p.append(i.display_order)
	    l=len(p)
	    print("llllllllllll")
	    print(l)
	    if(n[o]=='up'):
	      curr=i.display_order
	      var=l-2
	      pre=p[var]
	      print("curr")
	      print(curr)
	      ...
	      print(pre)
	      #i.display_order=pre
	      k=DBSession.query(project_items).filter_by(display_order=pre).first()
	      print("kkk")
	      m=k.display_order
	      i.display_order=m
	      k.display_order=curr
	      
	    elif(n[o]=='down'):
	     curr=i.display_order
	     j=o+1
	     
	     k=DBSession.query(project_items).filter_by(pi_id=ii[j]).first()
	     print("next")
	     nextt=k.display_order
	     i.display_order=nextt
	     k.display_order=curr
